...
# ---------------------------------------------------------------------------
# Detección de período histórico
# ---------------------------------------------------------------------------

#: Períodos canónicos en orden cronológico y sus palabras clave de detección.
PERIODOS_CANONICOS = [
    ("Prehispánica (~300 a.C. — 1517)", [
        "prehispán", "preclásico", "clásico temprano", "clásico tardío",
        "postclásico", "maya", "chenes", "xtampak", "hochob", "dzibilnocac",
        "mesoameric", "prehispan", "precolombin", "300 a.c", "a.c.", "a. c.",
        "prehispanica", "prehispanico",
    ]),
    ("Colonial temprana (1517 — 1669)", [
        "conquista", "colonial tempr", "montejo", "encomienda", "evangeliz",
        "franciscan", "visita eclesiastica", "visita eclesiástica", "1517",
        "1527", "1546", "1547", "siglo xvi", "siglo xvii",
        "colonial temprana", "colonial temprano",
    ]),
    ("Colonial tardía — Independencia — República — Porfiriato (1670 — 1910)", [
        "colonial tard", "independencia", "república", "republica",
        "reforma", "porfiriato", "porfiri", "1670", "1810", "1821",
        "siglo xviii", "siglo xix", "hacienda", "casta",
        "guerra de castas", "colonial tardia",
    ]),
    ("Revolución — Reforma Agraria — Economía chiclera (1910 — 1970)", [
        "revolución", "revolucion", "reforma agraria", "chicle", "chiclera",
        "ejido", "cardenas", "cárdenas", "lázaro", "lazaro", "1910",
        "1917", "1930", "1940", "1950", "1960", "siglo xx",
    ]),
    ("Contemporánea (1970 — 2026)", [
        "contempor", "neoliberal", "menonita", "transgénico", "transgenico",
        "leydy pech", "resistencia activa", "1970", "1980", "1990",
        "2000", "2010", "2020", "2026", "siglo xxi",
    ]),
    ("Transversal — todas las eras", [
        "transversal", "todas las eras", "todas las épocas",
        "demografía", "demografia", "rutas", "territorio",
    ]),
]
...
def detectar_periodo(
    ...
    """
    Detecta el período histórico canónico a partir de los metadatos de un registro.

    Examina en orden: ``era``, ``periodo``, ``categoria``, ``fecha``,
    ``titulo`` y ``descripcion``. Devuelve el primer período canónico cuyas
    palabras clave aparezcan en alguno de esos campos.

    Args:
        era (str): Campo ``era`` del archivo (p. ej. "Prehispánica").
        periodo (str): Campo ``periodo`` del registro.
        categoria (str): Campo ``categoria`` del registro.
        fecha (str): Campo de fecha en texto libre (p. ej. "1527–1547").
        titulo (str): Título del registro o documento.
        descripcion (str): Texto descriptivo del registro.

    Returns:
        str: Nombre del período canónico, o ``"Sin período definido"`` si no
             se puede determinar.
    """
    texto_combinado = " ".join([
        str(era), str(periodo), str(categoria), str(fecha),
        str(titulo), str(descripcion),
    ]).lower()

    for nombre_periodo, palabras_clave in PERIODOS_CANONICOS:
        for clave in palabras_clave:
            if clave.lower() in texto_combinado:
                return nombre_periodo

    # Intento final: extraer el primer año de 4 dígitos del campo fecha
    match = re.search(r"\b(\d{4})\b", str(fecha))
    if match:
        anio = int(match.group(1))
        if anio <= 1517:
            return PERIODOS_CANONICOS[0][0]
        if anio <= 1669:
            return PERIODOS_CANONICOS[1][0]
        if anio <= 1910:
            return PERIODOS_CANONICOS[2][0]
        if anio <= 1970:
            return PERIODOS_CANONICOS[3][0]
        return PERIODOS_CANONICOS[4][0]
