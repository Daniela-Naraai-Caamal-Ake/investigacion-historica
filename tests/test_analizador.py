...
# ---------------------------------------------------------------------------
# Pruebas de detectar_periodo
# ---------------------------------------------------------------------------

from utilidades import detectar_periodo, PERIODOS_CANONICOS  # noqa: E402


class TestDetectarPeriodo(unittest.TestCase):

    def test_prehispanico_por_era(self):
        resultado = detectar_periodo(era="Prehispánica")
        self.assertIn("Prehispán", resultado)

    def test_prehispanico_por_anio(self):
        resultado = detectar_periodo(fecha="300 a.C.")
        self.assertIn("Prehispán", resultado)

    def test_colonial_temprana_por_palabra_clave(self):
        resultado = detectar_periodo(era="Colonial temprana")
        self.assertIn("Colonial temprana", resultado)

    def test_colonial_tardia_por_porfiriato(self):
        resultado = detectar_periodo(periodo="Porfiriato")
        self.assertIn("Porfiriato", resultado)

    def test_revolucion_por_chicle(self):
        resultado = detectar_periodo(periodo="Economía chiclera")
        self.assertIn("chiclera", resultado)

    def test_contemporanea_por_anio_cuatro_digitos(self):
        resultado = detectar_periodo(fecha="1985")
        self.assertIn("Contemporánea", resultado)

    def test_sin_periodo(self):
        resultado = detectar_periodo()
        self.assertEqual(resultado, "Sin período definido")

    def test_anio_1700_colonial_tardio(self):
        resultado = detectar_periodo(fecha="1700")
        self.assertIn("Colonial tard", resultado)

    def test_anio_1920_revolucion(self):
        resultado = detectar_periodo(fecha="1920")
        self.assertIn("Revolución", resultado)

    def test_periodos_canonicos_no_vacio(self):
        self.assertGreater(len(PERIODOS_CANONICOS), 0)


# ---------------------------------------------------------------------------
# Pruebas de extraer_entradas_por_periodos
# ---------------------------------------------------------------------------

import analizador  # noqa: E402


class TestExtraerEntradasPorPeriodos(unittest.TestCase):

    def setUp(self):
        self.directorio_temp = tempfile.mkdtemp()

    def _crear_json(self, nombre, datos):
        ruta = os.path.join(self.directorio_temp, nombre)
        with open(ruta, "w", encoding="utf-8") as f:
            json.dump(datos, f, ensure_ascii=False)
        return ruta

    def _crear_md(self, nombre, contenido):
        ruta = os.path.join(self.directorio_temp, nombre)
        with open(ruta, "w", encoding="utf-8") as f:
            f.write(contenido)
        return ruta

    def test_nodo_json_genera_entradas(self):
        datos = {
            "era": "Prehispánica",
            "rango_temporal": "300 a.C. — 1517",
            "titulo": "Nodo test",
            "registros": [
                {
                    "registro_id": "T-001",
                    "subtitulo": "Primer asentamiento",
                    "fecha_evento": "300 a.C.",
                    "lugar": "Hopelchén",
                    "descripcion": "Descripción larga con suficientes caracteres para ser incluida en el reporte.",
                    "fuente": {"nombre": "Fuente Prueba", "url": "http://ejemplo.com"},
                }
            ],
        }
        ruta = self._crear_json("nodo_test.json", datos)
        por_periodo = analizador.extraer_entradas_por_periodos([ruta])
        self.assertGreater(sum(len(v) for v in por_periodo.values()), 0)
        periodos_con_datos = list(por_periodo.keys())
        self.assertTrue(any("Prehispán" in p for p in periodos_con_datos))

    def test_json_generico_genera_entradas(self):
        datos = {
            "coleccion": "Eventos",
            "eventos": [
                {
                    ...
                    "fecha": "1810",
                    "descripcion": "Descripción del evento histórico con suficiente longitud de texto.",
                    "periodo": "Independencia",
                    "fuentes": ["Fuente 1"],
                }
            ],
        }
        ruta = self._crear_json("eventos_test.json", datos)
        por_periodo = analizador.extraer_entradas_por_periodos([ruta])
        total = sum(len(v) for v in por_periodo.values())
        self.assertGreater(total, 0)

    def test_md_genera_entradas(self):
        contenido = "# Título del documento\n\n## Sección prehispánica\n\n" + "Texto de la sección " * 10
        ruta = self._crear_md("doc_test.md", contenido)
        por_periodo = analizador.extraer_entradas_por_periodos([ruta])
        total = sum(len(v) for v in por_periodo.values())
        self.assertGreater(total, 0)

    def test_filtro_longitud_minima(self):
        datos = {
            "eventos": [
                {"titulo": "X", "descripcion": "corto", "fecha": "1500", "fuentes": []},
            ]
        }
        ruta = self._crear_json("corto_test.json", datos)
        por_periodo = analizador.extraer_entradas_por_periodos([ruta], longitud_minima=50)
        total = sum(len(v) for v in por_periodo.values())
        self.assertEqual(total, 0)

    def test_archivo_inexistente_no_rompe(self):
        # Should not raise; should skip the missing file
        por_periodo = analizador.extraer_entradas_por_periodos(
            ["/tmp/archivo_que_no_existe.json"]
        )
        self.assertIsInstance(por_periodo, dict)

    def test_entradas_tienen_campos_requeridos(self):
        datos = {
            "era": "Colonial temprana",
            "rango_temporal": "1517 — 1669",
            "titulo": "Conquista",
            "registros": [
                {
                    "registro_id": "C-001",
                    "subtitulo": "Montejo llega",
                    "fecha_evento": "1527",
                    "lugar": "Yucatán",
                    "descripcion": "Los españoles llegaron a la península en el año 1527 con tres campañas.",
                    "fuente": {"nombre": "Chamberlain", "url": "http://ejemplo.com"},
                }
            ],
        }
        ruta = self._crear_json("colonial_test.json", datos)
        por_periodo = analizador.extraer_entradas_por_periodos([ruta])
        entradas = [e for lista in por_periodo.values() for e in lista]
        self.assertGreater(len(entradas), 0)
        for campo in ("periodo", "texto", "fuente", "subtitulo", "origen"):
            self.assertIn(campo, entradas[0])


# ---------------------------------------------------------------------------
# Pruebas de guardar_extraccion_periodos
# ---------------------------------------------------------------------------

class TestGuardarExtraccionPeriodos(unittest.TestCase):

    def setUp(self):
        self.directorio_temp = tempfile.mkdtemp()

    def test_genera_json_y_md(self):
        por_periodo = {
            "Prehispánica (~300 a.C. — 1517)": [
                {
                    "periodo": "Prehispánica (~300 a.C. — 1517)",
                    "tipo": "registro",
                    "subtitulo": "Asentamiento",
                    "fecha": "300 a.C.",
                    "lugar": "Hopelchén",
                    "texto": "Texto de prueba largo para la sección prehispánica.",
                    "fuente": "INAH",
                    "origen": "test.json",
                }
            ]
        }
        ruta_md, ruta_json = analizador.guardar_extraccion_periodos(
            por_periodo, self.directorio_temp
        )
        self.assertTrue(os.path.exists(ruta_json))
        self.assertTrue(os.path.exists(ruta_md))

    def test_json_tiene_estructura_correcta(self):
        por_periodo = {
            "Colonial temprana (1517 — 1669)": [
                {
                    "periodo": "Colonial temprana (1517 — 1669)",
                    "tipo": "registro",
                    "subtitulo": "Conquista",
                    "fecha": "1527",
                    "lugar": "Yucatán",
                    "texto": "Texto colonial de prueba con suficiente longitud.",
                    "fuente": "Chamberlain",
                    "origen": "colonial.json",
                }
            ]
        }
        ruta_md, ruta_json = analizador.guardar_extraccion_periodos(
        ...
        with open(ruta_json, encoding="utf-8") as f:
            datos = json.load(f)
        self.assertIn("fecha_generacion", datos)
        self.assertIn("total_entradas", datos)
        self.assertIn("periodos", datos)
        self.assertEqual(datos["total_entradas"], 1)

    def test_md_contiene_periodo_y_texto(self):
        por_periodo = {
            "Prehispánica (~300 a.C. — 1517)": [
                {
                    "periodo": "Prehispánica (~300 a.C. — 1517)",
                    "tipo": "registro",
                    "subtitulo": "Sitio arqueológico",
                    "fecha": "500 a.C.",
                    "lugar": "Xtampak",
                    "texto": "Texto prehispánico de prueba con suficiente longitud.",
                    "fuente": "INAH 2024",
                    "origen": "nodo1.json",
                }
            ]
        }
        ruta_md, _ = analizador.guardar_extraccion_periodos(
            por_periodo, self.directorio_temp
        )
        with open(ruta_md, encoding="utf-8") as f:
            contenido = f.read()
        self.assertIn("Prehispánica", contenido)
        self.assertIn("Texto prehispánico", contenido)
        self.assertIn("INAH 2024", contenido)

    def test_directorio_creado_si_no_existe(self):
        nuevo_dir = os.path.join(self.directorio_temp, "subdir_nuevo")
        self.assertFalse(os.path.exists(nuevo_dir))
        por_periodo = {
            "Sin período definido": [
                {
                    ...
                    "fecha": "",
                    "lugar": "",
                    "texto": "Texto de prueba sin período definido.",
                    "fuente": "",
                    "origen": "x.json",
                }
            ]
        }
        analizador.guardar_extraccion_periodos(por_periodo, nuevo_dir)
