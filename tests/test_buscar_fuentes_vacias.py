...
class TestIntegracionArchivoReal(unittest.TestCase):
    ...
    def test_detecta_registros_conocidos_sin_fuente(self):
        """Todos los registros en los nodos reales deben tener campo fuente."""
        entradas = bfv.detectar_registros_sin_fuente()
        ids_encontrados = {e["registro"]["registro_id"] for e in entradas}
        # Todos los registros tienen ya campo fuente (aunque sea placeholder).
        # Si en el futuro se agregan registros sin fuente, este test falla para
        # que el autor los documente explícitamente.
        self.assertEqual(
            ids_encontrados, set(),
            f"Registros sin fuente encontrados (deben tener fuente): {ids_encontrados}",
        )
